"""Juniper Mist API integration helpers for tenant-aware polling and discovery.

``MistClient`` wraps the static-token request logic needed by the cs spoke
background poller. It mirrors ``aruba.ArubaClient``'s public surface
(``poll_site_data`` / ``browse_all`` / ``available_checks`` / ``list_sites`` /
``list_clients``) and returns the SAME dict shapes so the existing
``mist_poller.MistPoller`` (a near-twin of ``central_poller.CentralPoller``) can
reuse the data-source-agnostic ``ClientCountTracker`` / ``CheckHealthHistory``
helpers and write ``spoke.mist_status`` in the exact ``central_status`` shape
``sim-views.js``'s Checks/Hardware/Client-Count tabs already render.
...
# 1-day window when omitted, which hides any alarm older than 24h — so the
# browse/picker paths pass a wider window to surface recent history, while the
...
class MistClient:
    ...
    async def poll_site_data(
        ...
        # The dashboard active-alarm poll keeps the 1d default (current problems
        # only) and ignores the fetch-warning — counts stay as-is on failure.
    ...
    async def available_checks(self) -> dict[str, Any]:
        ...
        warnings: list[str] = []
        try:
            # Use the wider browse window so the picker offers the full universe
            # of recent alarm types, not just the last 24h.
