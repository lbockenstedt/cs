...
# ── USB link-speed cap (force the dongle to USB 2.0) ────────────────────────
# Several SuperSpeed dongles on one controller oversubscribe the bus and
# throughput collapses for all of them. Capping each device at USB 2.0 (480
# Mbps) is a natural throttle that keeps the shared bus inside its budget.
#
# HOW: a USB3 port is two link sets sharing one physical connector. Disabling
# the SuperSpeed side makes the device re-enumerate on its USB 2.0 companion.
# The kernel exposes that companion via the port's `peer` symlink, so the target
# bus is read rather than guessed.
#
# WHERE: this only bites where the guest owns a REAL controller — a T1/T3 PCI
# passthrough. On a T2 client the controller is emulated and Proxmox already
# presents USB 2.0, so the speed check below simply finds nothing to do.
#
# SAFETY: this dongle is the client's ONLY link. If it does not come back on the
# companion bus the port is RE-ENABLED and we continue at USB3 — a client that
# loses its NIC here is off the fleet with no remote way back in, which is far
# worse than an uncapped link.
usb_force_usb2_enabled() {
  local v; v=$(get_value 'simulation' 'force_usb2' 2>/dev/null)
...
# Does this USB device own a wl* netdev? Plain glob rather than `compgen -G`:
# an unmatched glob leaves the pattern literal and the -e test simply fails, so
# this behaves the same with or without nullglob. A false negative here would
# silently cap nothing, which is exactly the kind of quiet no-op worth avoiding.
_usb_has_wifi() {
  local x
  for x in "$1"/*/net/wl*; do
    [[ -e "$x" ]] && return 0
  done
  return 1
}

# Any wifi interface present at all?
_usb_any_wifi_iface() {
  local x
  for x in /sys/class/net/wl*; do
    [[ -e "$x" ]] && return 0
  done
  return 1
}

# USB device dirs that own a wl* netdev (i.e. the wifi dongles, not hubs).
_usb_wifi_devs() {
  local d
  for d in /sys/bus/usb/devices/*-*; do
    [[ -f "$d/speed" ]] || continue
    _usb_has_wifi "$d" && echo "$d"
  done
}

usb_force_usb2() {
  if ! usb_force_usb2_enabled; then
    echo "  [usb2] force_usb2=off — leaving link speed alone" | tee -a "${LOG_FILE:-/dev/null}"
    return 0
  fi
  local d sp dev bus port pd peer pname pbus pport expect i
  for d in $(_usb_wifi_devs); do
    sp=$(cat "$d/speed" 2>/dev/null)
    [[ "$sp" =~ ^[0-9]+$ ]] || continue
    dev=$(basename "$d")
    if (( sp < 5000 )); then
      echo "  [usb2] $dev already ${sp}Mbps — no change" | tee -a "${LOG_FILE:-/dev/null}"
      continue
    fi
    bus=${dev%%-*}; port=${dev##*-}
    pd="/sys/bus/usb/devices/usb${bus}/${bus}-0:1.0/usb${bus}-port${port}"
    if [[ ! -e "$pd/disable" ]]; then
      echo "  [usb2] $dev: no port control at $pd — skipped" | tee -a "${LOG_FILE:-/dev/null}"
      continue
    fi
    # Read the companion port so we know exactly where it should reappear.
    expect=""
    peer=$(readlink -f "$pd/peer" 2>/dev/null || true)
    if [[ -n "$peer" ]]; then
      pname=$(basename "$peer")            # usb9-port3
      pbus=${pname#usb}; pbus=${pbus%%-port*}
      pport=${pname##*-port}
      [[ "$pbus" =~ ^[0-9]+$ && "$pport" =~ ^[0-9]+$ ]] && expect="/sys/bus/usb/devices/${pbus}-${pport}"
    fi
    echo "  [usb2] $dev at ${sp}Mbps — disabling SuperSpeed port to force USB2${expect:+ (expect ${expect##*/})}" \
      | tee -a "${LOG_FILE:-/dev/null}"
    if ! sudo -n sh -c "echo 1 > '$pd/disable'" 2>/dev/null; then
      echo "  [usb2] WARNING: cannot write $pd/disable (no sudo?) — skipped" | tee -a "${LOG_FILE:-/dev/null}"
      continue
    fi
    # Wait for it on the COMPANION bus specifically. Checking for "any wl*"
    # would be satisfied by a different dongle that never moved, hiding a
    # failure until the client is already offline.
    for (( i = 0; i < 15; i++ )); do
      sleep 1
      if [[ -n "$expect" ]]; then
        [[ -d "$expect" ]] && break
      else
        _usb_any_wifi_iface && break
      fi
    done
    if { [[ -n "$expect" && -d "$expect" ]]; } || { [[ -z "$expect" ]] && _usb_any_wifi_iface; }; then
      echo "  [usb2] OK — now ${expect:+$(cat "$expect/speed" 2>/dev/null)}${expect:-}Mbps" | tee -a "${LOG_FILE:-/dev/null}"
    else
      echo "  [usb2] FAILED — dongle did not return after 15s; RE-ENABLING port (staying USB3)" \
        | tee -a "${LOG_FILE:-/dev/null}"
      sudo -n sh -c "echo 0 > '$pd/disable'" 2>/dev/null || true
      sleep 3
    fi
  done
}
